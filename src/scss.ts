...
module.exports = function (config: IConfig) {
    ...
    const distDir = `${utils.get(config, 'outputDirPath')}/${utils.get(config, 'scss.outputDirName')}`;

    const getTimeString = () => (new Date).toLocaleTimeString('ja', {timeZone: `${utils.get(config, 'timeZone')}`});

    if (!fs.existsSync(`${utils.get(config, 'cacheDirPath')}`))
        fs.mkdirSync(`${utils.get(config, 'cacheDirPath')}`);
    ...
    const convert = (fileName, srcDir, distDir) => {
        ...
        const result = sass.renderSync({
                                           ...
                                           outputStyle:    isProd ? `${utils.get(config, 'scss.outputStyle.production')}` : `${utils.get(config, 'scss.outputStyle.development')}`,
                                       });
        let css = result.css.toString();
        let cssMap = result.map;

        let cache = fs.existsSync(`${utils.get(config, 'cacheDirPath')}/scss.json`) ? JSON.parse(fs.readFileSync(`${utils.get(config, 'cacheDirPath')}/scss.json`).toString()) : null;
        ...
        fs.writeFileSync(`${utils.get(config, 'cacheDirPath')}/scss.json`, JSON.stringify(newCache));
    ...
    if (isWatch) {
        const watcher = chokidar.watch(`${utils.get(config, 'inputDirPath')}/${utils.get(config, 'scss.inputDirName')}/**/*.scss`, {
            usePolling: true,
        });
        ...
        watcher.on('change', changePath => {
            try {
                if (/_(.*).scss/.test(path.basename(changePath))) {
                    const cache = fs.existsSync(`${utils.get(config, 'cacheDirPath')}/scss.json`) ? JSON.parse(fs.readFileSync(`${utils.get(config, 'cacheDirPath')}/scss.json`).toString()) : null;
                    if (cache !== null && cache.files !== null) {
